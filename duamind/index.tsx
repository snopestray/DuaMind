...
const StarIcon = ({ filled }) => (
    <svg xmlns="http://www.w.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" fill={filled ? '#FFC107' : 'currentColor'} style={{ display: 'block' }}>
...
const DeleteIcon = () => (
     <svg xmlns="http://www.w.org/2000/svg" height="20" viewBox="0 -960 960 960" width="20" fill="currentColor"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
);
...
const GeneratorResult = ({ dua, showSafetyNotice, handleSaveDua, handleSaveAsImage, handleSaveAsPdf, setGeneratorView, setError }) => {
...
// --- GLOBAL SNACKBAR STATE (outside component to be accessible everywhere) ---
let setSnackbar;
...
const App = () => {
    ...
    setSnackbar = setSnackbarMessage; // Assign setter to global variable
    ...
    const handleSaveDua = () => {
        ...
        setSnackbar('Gebet im Gebetsbuch gespeichert');
    };
    ...
    const handleSaveAsImage = async (duaDetails) => {
        ...
        try {
            ...
            link.click();
            setSnackbar('Bild wird heruntergeladen...');
        } catch (err) {
            console.error('Image generation failed:', err);
            setSnackbar('Fehler bei der Bilderstellung.');
        } finally {
    ...
    const handleSaveAsPdf = async (duaDetails) => {
        ...
        try {
            ...
            setSnackbar('PDF wird heruntergeladen...');
        } catch(err) {
            console.error('PDF generation failed:', err);
            setSnackbar('Fehler bei der PDF-Erstellung.');
        } finally {
            document.body.removeChild(element);
        }
    };
    ...
    return (
        <div style={styles.appContainer}>
            ...
            {mainView === 'generator' && (
                ...
                : <GeneratorResult dua={generatedDua} showSafetyNotice={showSafetyNotice} handleSaveDua={handleSaveDua} handleSaveAsImage={imageExportHandler} handleSaveAsPdf={pdfExportHandler} setGeneratorView={setGeneratorView} setError={setError} />
            )}
...
textarea:focus { border-color: #2F6B5A !important; outline: none; }
textarea::placeholder { color: #666666; opacity: 1; font-style: italic; }
.primary-button:hover:not(:disabled) { background-color: #2F6B5A !important; }
.secondary-button:hover:not(:disabled) { border-color: #3D7C67 !important; color: #2F6B5A !important; }
.prayerBookCard:hover { border-color: #3D7C67; }
.deleteButton:hover { background-color: #FEE2E2; color: #D32F2F; }
`;
